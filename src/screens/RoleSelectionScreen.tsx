...
export default function RoleSelectionScreen() {
  const { theme, setRole, loginAsAdmin, loginAsManagement, loginAsTeam, tenantId, logout } = useAppContext();
  ...
  const [loginTarget, setLoginTarget] = useState<any>(null);

  useEffect(() => {
    if (!tenantId) return;
    const unsubTeams = onSnapshot(query(collection(db, 'teams'), where('tenantId', '==', tenantId)), (snap) => {
      setTeams(snap.docs.map(d => ({ id: d.id, ...d.data() })));
    });
    const unsubConfig = onSnapshot(doc(db, 'admin', tenantId), (docSnap) => {
  ...
  const handleSubmitPin = () => {
    ...
    if (loginTarget.type === 'admin') {
      if (!adminConfig.pinEnabled || pin === (adminConfig.pin || '1234')) {
        loginAsAdmin();
      } else {
  ...
  return (
    <View style={styles.container}>
      <ScrollView contentContainerStyle={styles.scroll} keyboardShouldPersistTaps="handled">
        ...
        <View style={styles.card}>
          {showPinInput ? (
            ...
            <View>
              ...
              {teams.map(team => (
                <TouchableOpacity key={team.id} style={[styles.mgmtBtn, { borderColor: theme.primaryColor }]} onPress={() => initiateLogin({ type: 'team', team })}>
                  <View style={styles.btnInner}>
                    <Text style={styles.btnEmoji}>👤</Text>
                    <View>
                      <Text style={styles.mgmtBtnTitle}>{team.name}</Text>
                      <Text style={styles.mgmtBtnSub}>Perfil de equipo</Text>
                    </View>
                  </View>
                  <Text style={styles.chevron}>›</Text>
                </TouchableOpacity>
        ...
        <TouchableOpacity style={{ padding: 15, alignItems: 'center', marginTop: 20 }} onPress={logout}>
          <Text style={{ color: '#e74c3c', fontWeight: 'bold' }}>Cerrar Sesión</Text>
        </TouchableOpacity>
